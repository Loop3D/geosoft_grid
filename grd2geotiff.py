...
# command line version of convertor hacked togetehr by M. Jessell
...
#
# This code is modefiied from  the Fatiando a Terra project (https://www.fatiando.org)
#
...
# ### Code to convert extracted xarray to geotiff format file
...
def write_geotiff(out_path,grid_uc,epsg):
    """
    write out grid in geotiff format
    Parameters
    ----------
    out_path : string
        file path and name to write to
    grid_uc : xarray
        grid and header info  
    epsg: string
        user defined epsg
    """    
    transform = from_origin(grid_uc.attrs['x_origin']-(grid_uc.attrs['spacing_e']/2), grid_uc.attrs['y_origin']-(grid_uc.attrs['spacing_v']/2), grid_uc.attrs['spacing_e'], -grid_uc.attrs['spacing_v'])

    new_dataset = rasterio.open(
        out_path,
        "w",
        driver="GTiff",
        height=grid_uc.attrs['shape_v'],
        width=grid_uc.attrs['shape_e'],
        count=1,
        dtype=rasterio.float32,
        crs='epsg:'+epsg,#"+proj=longlat",
        transform=transform,
        nodata=-1e8,
    )

    new_dataset.write(grid_uc.values, 1)
    new_dataset.close()
    print("dtm geotif saved as", out_path)


# ### Example code to convert uncompressed grid
# Usage python grd2geotiff.py input_grid_path epsg
...
if(len(sys.argv)<3):
    print('usage: python grd2geotiff.py input_grid_path [string] epsg [int]')
    print('       e.g. python grd2geotiff.py test_data/test_compressed.grd 4326')
    print()
    print('output: input filename root with tif suffix')
    exit()
filename=str(sys.argv[1])
filename_out=filename.replace('.grd','.tif')
epsg=str(sys.argv[2])

grid_uc=load_oasis_montaj_grid(filename)
write_geotiff(filename_out,grid_uc,epsg)
...
#proj = extract_proj_str(filename+".xml")
